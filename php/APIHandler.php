...
$uri = explode('/', $uri);
$uri_controller = isset($uri[2]) ? $uri[2] : "";
$uri = explode('.php', $uri[1]);
$uri = $uri[0];

$uri_controller_allows = ['ChangePassword.php', 'AddLoadLeads.php'];
if ($uri != 'index')
{
    if ($uri != 'login')
    {
        ...
        {
            if ($uri == 'php' && !in_array($uri_controller, $uri_controller_allows)) die("This file cannot be accessed directly");
        }
    }
}
...
class APIHandler
{
    ...
    */
    public function API_Request($folder, $postfields, $request_data = false)
    {
        ...
        if ($request_data === true) return $data;
        else return $output;
    }
    ...
    */
    public function API_Upload($folder, $postfields, $return_data = NULL)
    {
        ...
        if (!empty($return_data)) return array(
            "output" => $output,
            "data" => $data,
            "URL" => $url,
            "CONNECTION" => $postdata
    ...
    public function goGetPermissions($type = 'dashboard')
    {
        ...
        $decoded_permission = json_decode($permissions
            ->data
            ->permissions);
    ...
    {
        $postfields = array(
            'goAction' => 'goGetAllCategories'
        );
        return $this->API_Request("goCategories", $postfields);
    }

    public function API_getLeadDispositions($lead_id = null)
    {
        $postfields = array(
            'goAction' => 'goGetLeadDispositions',
            'lead_id' => $lead_id
        );
        return $this->API_Request("goDispositions", $postfields);
    }
    ...
    */
    ...
    public function API_getAssignedUsers($campaign_id = null)
    {
        $postfields = array(
            'goAction' => 'goGetAssignedUsers',
            'campaign_id' => $campaign_id
        );
        return $this->API_Request("goUsers", $postfields);
    }

    public function API_assignUsers($postfields)
    {
    ...
    public function API_getUserGroups()
    {
        $postfields = array(
            'goAction' => 'goGetUserGroups'
        );
        return $this->API_Request("goUserGroups", $postfields);
    }
