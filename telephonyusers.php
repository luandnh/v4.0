...
<head>
	...
	<link rel="stylesheet" href="css/daterangepicker/daterangepickerv3.css">
	</link>
</head>
						...
						<fieldset>
							...
							<div class="form-group">
								...
								<div class="col-sm-8 mb">
									<input type="text" class="form-control" name="user_form" id="user_form" placeholder="<?php $lh->translateText("user_id"); ?> (<?php $lh->translateText("mandatory"); ?>)" value="<?php echo $user_id_for_form; ?>" title="<?php $lh->translateText("alphanumberic_only_instruction"); ?>" maxlength="20" required>
							...
							<div class="form-group" id="phone_logins_form" style="display:none;">
								...
								<div class="col-sm-8 mb">
									<input type="number" name="phone_logins" id="phone_logins" class="form-control" minlength="3" placeholder="<?php $lh->translateText("phone_login"); ?> (<?php $lh->translateText("mandatory"); ?>)" value="<?php echo $phones->available_phone; ?>" pattern=".{3,}" title="Minimum of 3 characters" maxlength="20" required>
							...
							<div class="form-group">
								...
								<div class="col-sm-8 mb">
									<input type="text" name="fullname" id="fullname" class="form-control" placeholder="<?php $lh->translateText("full_name"); ?> (<?php $lh->translateText("mandatory"); ?>)" value="<?php echo $fullname; ?>" title="<?php $lh->translateText("alphanumberic_only_instruction"); ?>" maxlength="50" required>
								</div>
							</div>
							<div class="form-group" id="password_div">
								<label class="col-sm-4 control-label" for="password"><i class="fa fa-info-circle" title="<?php $lh->translateText("default_pass_is"); ?>: <?php $lh->translateText("brand_title"); ?><?php echo date('Y'); ?>"></i> <?php $lh->translateText("password"); ?> </label>
								<div class="col-sm-8 mb">
									<div class="input-group" data-children-count="2">
										<input type="password" class="form-control" name="password" id="password" placeholder="Password (<?php $lh->translateText("mandatory"); ?>)" value="<?php $lh->translateText("brand_title"); ?><?php echo date('Y'); ?>" maxlength="20" required>
										<span class="input-group-addon" data-children-count="1" title="Show Password">
							...
							<div class="form-group" id="confirm_div">
								...
								<div class="col-sm-8 mb">
									<input type="password" class="form-control" id="confirm" name="confirm" placeholder="<?php $lh->translateText("reenter_pass"); ?> (<?php $lh->translateText("mandatory"); ?>)" value="<?php $lh->translateText("brand_title"); ?><?php echo date('Y'); ?>" required>
								</div>
							</div>
							<div class="form-group">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".bottom-menu").on('mouseenter mouseleave', function() {
			$(this).find(".fab-div-area").stop().slideToggle({
				height: 'toggle',
				opacity: 'toggle'
			}, 'slow');
		});
			...
			onFinished: function(event, currentIndex) {
				...
				$.ajax({
					...
					success: function(data) {
						...
						if (data == 1) {
							swal({
								title: "<?php $lh->translateText("add_phone_success"); ?>",
								text: "<?php $lh->translateText("phone_has_been_saved"); ?>",
								type: "success"
							}, function() {
								window.location.href = 'telephonyusers.php?phone_tab';
							});
						} else {
					...
					if (isConfirm) {
						$.ajax({
							...
							success: function(data) {
								...
								if (data == 1) {
									swal({
										title: "<?php $lh->translateText("delete_user_success"); ?>",
										text: "<?php $lh->translateText("user_has_been_deleted"); ?>",
										type: "success"
									}, function() {
										window.location.href = 'telephonyusers.php';
									});
					...
					if (isConfirm) {
						$.ajax({
							...
							success: function(data) {
								...
								if (data == 1) {
									swal({
										title: "<?php $lh->translateText("delete_phone_success"); ?>",
										text: "<?php $lh->translateText("phone_has_been_deleted"); ?>",
										type: "success"
									}, function() {
										window.location.href = 'telephonyusers.php?phone_tab';
									});
		...
		$(document).on('change', '#user_group', function() {
			if ($("#user_group").val() == "ADMIN" || $("#user_group").val() == "SUPERVISOR") {
				...
				$("#password").val('<?php $lh->translateText("brand_title"); ?>' + <?php echo date("Y"); ?>);
			} else {
				$("#password").attr('maxlength', '20');
				$("#password").val('<?php $lh->translateText("brand_title"); ?>' + <?php echo date("Y"); ?>);
			}
