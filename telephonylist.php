				...
				<div class="row">
										...
										<div id="dnc_tab" role="tabpanel" class="tab-pane <?php if (isset($_GET['dnc_tab'])) echo 'active' ?>">
											<table class="display responsive no-wrap table table-striped table-bordered" width="100%" id="table_dnc">
												...
												<tbody>
													<tr id="dnc_result">
														<td colspan="3">
															<center><span id="dnc_error">- - - <?php $lh->translateText("search_filter_dnc"); ?> - - -</span></center>
														</td>
														<td></td>
														<td></td>
													</tr>
					...
					<div class="col-lg-3" id="list_sidebar">
						...
						<form action="./php/AddLoadLeads.php" method="POST" enctype="multipart/form-data" id="upload_form" name="upload_form">
								...
								<div id="LeadMappingContainer" class="modal" tabindex="-1" role="dialog" data-keyboard="false" data-backdrop="static">
									<div class="modal-dialog" role="document">
										<div class="modal-content">
											...
											<div class="modal-footer" id="mapping_btns">
							...
							<div class="form-group" id="upload_form_btns">
								<input type="button" id="btnUpload" name="btnUpload" value="<?php $lh->translateText("update"); ?>" class="btn btn-primary" onClick="goProgressBar();">
...
<div id="list-uploaded-modal" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">
		<!-- Modal content-->
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title"><b><?php $lh->translateText("data_preview"); ?></b></h4>
			</div>
			<div class="modal-body">
				<table id="list-uploaded-tabled" class="display responsive no-wrap table table-responsive table-striped table-bordered" width="100%">
...
<script type="text/javascript">
						...
						if (isConfirm) {
							$.ajax({
								...
								success: function(data) {
									...
									if (data == 1) {
										swal({
											title: "<?php $lh->translateText("delete_list_success"); ?>",
											text: "<?php $lh->translateText("delete_list_success_msg"); ?>",
											type: "success"
										}, function() {
											window.location.href = 'telephonylist.php';
										});
					...
					$.ajax({
						...
						success: function(data) {
							...
							if (data == 1) {
								if (stageDNC == "ADD") {
									swal({
										title: "<?php $lh->translateText("added_new"); ?> DNC",
										text: "<?php $lh->translateText("add_dnc"); ?>",
										type: "success"
									}, function() {
										window.location.href = 'telephonylist.php?dnc_tab';
									});
								}
								if (stageDNC == "DELETE") {
									swal({
										title: "<?php $lh->translateText("deleted"); ?> DNC",
										text: "<?php $lh->translateText("delete_dnc"); ?>",
										type: "success"
									}, function() {
										window.location.href = 'telephonylist.php?dnc_tab';
									});
	...
	$('#LeadMapSubmit').on("change", () => {
		appendPreviewBtn()
	});
	$("#LeadMappingContainer").on('shown.bs.modal', () => {
		appendPreviewBtn()
	});
	let appendPreviewBtn = () => {
		if ($('#LeadMapSubmit').is(':checked')) {
			if ($("#preview_btn").length) {
				let elem = document.getElementById("preview_btn");
	...
	let showUploadedList = (data) => {
		...
		if ($('#LeadMapSubmit').is(':checked')) {
			...
			map_data = form.elements["map_data[]"];
			map_fields = form.elements["map_fields[]"];
			for (let index = 0; index < map_fields.length; index++) {
				field = map_fields[index];
				field_value = field.value
				tableColumns.push({
					"title": field_value
				})
				field_data = map_data[index];
				mapTable.push(field_data.value);
			}
			for (let index2 = 1; index2 < uploadData.length; index2++) {
				let row = uploadData[index2];
				if (row.length > 1) {
					let tmp = []
					mapTable.forEach((element) => {
						tmp_value = ""
						for (let sub_i = 0; sub_i < row.length; sub_i++) {
							if (element == sub_i) {
								tmp_value = row[sub_i]
							}
						}
					...
					data_table.push(Object.assign({}, tmp))
				}
			}
		} else {
			if (uploadData.length > 0) {
				let columns = uploadData[0];
				...
				for (let index2 = 1; index2 < uploadData.length; index2++) {
					...
					if (row.length >= columns.length) {
						data_table.push(Object.assign({}, row))
					}
				}
			}
		}
		let uploadedTable = $("#list-uploaded-tabled").DataTable({
			destroy: true,
			responsive: true,
			select: true,
			stateSave: true,
		...
		$('#list-uploaded-modal').on('hidden.bs.modal', function() {
			lead_mapping(temporary_res)
		});
		$("#list-uploaded-modal").modal("show");

		$("#list-uploaded-modal").on('shown.bs.modal', function() {
			uploadedTable.draw();
			uploadedTable.columns.adjust().responsive.recalc();
		});
