...
if ($output->result == "success") {
	...
	if ($pageTitle == "agent_detail") {
		...
		// top table
		$agent_detail .= '<div>
					<table class="display responsive no-wrap table table-striped table-bordered table-hover" width="100%" id="">
						<thead>
							<tr>
	...
	if ($pageTitle == "stats") {
		...
		if ($_POST["request"] == "daily") {
			$max = max(/*$output->getReports->data_calls->hour0, $output->getReports->data_calls->hour1, $output->getReports->data_calls->hour2, $output->getReports->data_calls->hour3, 
				...
				$output->getReports->data_calls->hour8,
				$output->getReports->data_calls->hour9,
				$output->getReports->data_calls->hour10,
				$output->getReports->data_calls->hour11,
				$output->getReports->data_calls->hour12,
				$output->getReports->data_calls->hour13,
				$output->getReports->data_calls->hour14,
				$output->getReports->data_calls->hour15,
				$output->getReports->data_calls->hour16,
				$output->getReports->data_calls->hour17,
				$output->getReports->data_calls->hour18,
				$output->getReports->data_calls->hour19,
				$output->getReports->data_calls->hour20,
				$output->getReports->data_calls->hour21,
				$output->getReports->data_calls->hour22
			);/*, $hour23);*/
		}
		if ($_POST["request"] == "weekly") {
			$max = max(
				$output->getReports->data_calls->Day0,
				$output->getReports->data_calls->Day1,
				$output->getReports->data_calls->Day2,
				$output->getReports->data_calls->Day3,
				$output->getReports->data_calls->Day4,
				$output->getReports->data_calls->Day5,
				$output->getReports->data_calls->Day6
			);
		}
		if ($_POST["request"] == "monthly") {
			$max = max(
				$output->getReports->data_calls->Month1,
				$output->getReports->data_calls->Month2,
				$output->getReports->data_calls->Month3,
				$output->getReports->data_calls->Month4,
				$output->getReports->data_calls->Month5,
				$output->getReports->data_calls->Month6,
				$output->getReports->data_calls->Month7,
				$output->getReports->data_calls->Month8,
				$output->getReports->data_calls->Month9,
				$output->getReports->data_calls->Month10,
				$output->getReports->data_calls->Month11,
				$output->getReports->data_calls->Month12
			);
		}
																							...
																							$monthname = "";
																								?> {
																									...
																									var options = {
																										...
																										yaxis: {
																											min: 0,
																											max: <?php echo $max_count; ?>, // optional: use it for a clear represetation
																											tickColor: '#eee',
