							...
							<form id="modifyvoicemail">
								...
								<div role="tabpanel">
									...
									<div class="form-group row">
										<label for="group_list_id" class="col-sm-2 control-label">Group List ID</label>
										<div class="col-sm-10 mb">
											<input type="text" class="form-control" name="group_list_id" id="group_list_id" placeholder="Group List ID" value="<?php //echo $output->data->group_list_id;
																																								?>" readonly>
										</div>
									</div>-->
											</fieldset>
										...
										<div id="tab_3" class="tab-pane fade in">
											<fieldset>
												...
												foreach ($perms as $type => $perm) {
													$hiddenRow = '';
													if (preg_match("/support|multi-tenant|chat|osticket/", $type)) {
														$hiddenRow = ' hidden';
													}
													...
													foreach ($perm as $idx => $value) {
														if (!preg_match("/^(moh_read)$/", $idx)) {
															...
															$disableThis = '';
															if (preg_match("/^(ADMIN|AGENTS)$/", $usergroup_id)) {
															...
															if (!preg_match("/dashboard|reportsanalytics|recordings|support/", $type)) {
																if (preg_match("/_create$/", $idx)) {
																	$defaultValue = 'C';
																	$label = 'Create';
																}
																if (preg_match("/_read$/", $idx)) {
																	$defaultValue = 'R';
																	$label = 'Read';
																}
																if (preg_match("/_update$/", $idx)) {
																	$defaultValue = 'U';
																	$label = 'Update';
																}
																if (preg_match("/_delete$/", $idx)) {
																	$defaultValue = 'D';
																	$label = 'Delete';
																}
																if (preg_match("/_upload$/", $idx)) {
																	$defaultValue = 'C';
																	$label = 'Upload';
																}
																if (preg_match("/_play/", $idx)) {
																	$defaultValue = 'Y';
																	$label = 'Play';
																}
																if (preg_match("/_download/", $idx)) {
																	$defaultValue = 'Y';
																	$label = 'Download';
																}
										...
										<div id="tab_4" class="tab-pane fade in">
											...
											<div class="row">
												<style>
													#filter_groups_multiselect {
														overflow: hidden;
														font-size: 17px;
													}

													#filter_groups_multiselect_to {
														overflow: hidden;
														font-size: 17px;
													}
												</style>
												<div class="col-md-5">
													<select name="groups_assign[]" id="filter_groups_multiselect_to" class="form-control" size="8" multiple="multiple"></select>
												</div>
												<div class="col-md-2">
													<button type="button" id="filter_groups_multiselect_rightAll" class="btn btn-outline-success btn-rounded btn-block"><i class="fa fa-angle-double-right"></i></button>
													<button type="button" id="filter_groups_multiselect_rightSelected" class="btn btn-outline-primary btn-rounded btn-block"><i class="fa fa-chevron-right"></i></button>
													<button type="button" id="filter_groups_multiselect_leftSelected" class="btn btn-outline-primary btn-rounded btn-block"><i class="fa fa-chevron-left"></i></button>
													<button type="button" id="filter_groups_multiselect_leftAll" class="btn btn-outline-success btn-rounded btn-block"><i class="fa fa-angle-double-left"></i></button>
												</div>
												<div class="col-md-5">
													<select id="filter_groups_multiselect" class="form-control" size="8" multiple="multiple">
													</select>
												</div>
											</div>
										</div><!-- tab 4 -->
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		let primary_usergroup = usergroups.find(element => element.user_group === usergroup_id);
		let manage_groups = [];
		if (primary_usergroup.sub_user_groups != null) {
			manage_groups = primary_usergroup.sub_user_groups.split(",");
		}
		usergroups.forEach((element) => {
			if (element.user_group == usergroup_id) {
				return
			} else if (manage_groups.includes(element.user_group)) {
		...
		$("#filter_groups_multiselect").append(group_option_from);
		$("#filter_groups_multiselect_to").append(group_option_to);
		$("#filter_groups_multiselect").multiselect({
			right: '#filter_groups_multiselect_to',
			rightAll: '#filter_groups_multiselect_leftAll',
			rightSelected: '#filter_groups_multiselect_leftSelected',
			leftSelected: '#filter_groups_multiselect_rightSelected',
			leftAll: '#filter_groups_multiselect_rightAll',
			search: {
