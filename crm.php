		...
		if ($("#search_customers").is(":checked") || $('#search_contacts').is(":checked")) {
			var tableContacts = $('#table_contacts').DataTable({
				...
				columns: [{
						data: "id",
						render: (data) => {
							return '<a class=\"edit-contact\" data-id=\"' + data + '\">' + data + '</a>';
						}
					},
					{
						data: "full_name"
	...
	$.fn.dataTable.pipeline = function(opts) {
		...
		return (request, callback, settings) => {
			...
			if (ajax) {
				...
				let data_counter = 1;
				let search_form = document.getElementById("search_form").elements;
				search_form["offset"].value = offset;
				search_form["limit"].value = limit;
				$.ajax({
